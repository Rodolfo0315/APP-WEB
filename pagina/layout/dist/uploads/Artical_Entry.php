...
if(isset($_POST['btn_sub'])){
    ...
    $stmt = mysqli_prepare($con, "INSERT INTO article_tbl (loca_id, title, content, status, note) VALUES (?, ?, ?, ?, ?)");
    // Vincula los parámetros a la sentencia preparada
    mysqli_stmt_bind_param($stmt, 'issss', $lid, $title, $content, $status, $note);
	// Ejecuta la sentencia preparada
    if (mysqli_stmt_execute($stmt))
        $msg = "1 Record inserted...";
    else
        $msg = "Insert Fail...".mysqli_error($con);
...
if(isset($_POST['btn_upd'])){
	...
	$note=$_POST['notetxt'];
	  ...
	  // Prepara la consulta SQL para evitar inyecciones SQL
      $stmt = mysqli_prepare($con, "UPDATE article_tbl SET loca_id = ?, title = ?, content = ?, status = ?, note = ? WHERE a_id = ?");
    
      // Vincula los parámetros a la sentencia preparada
      mysqli_stmt_bind_param($stmt, 'issssi', $loca_id, $title, $content, $status, $note, $id);
      
      // Ejecuta la sentencia preparada
      if (mysqli_stmt_execute($stmt))
          header("location:?tag=view_artical");
      else
          $msg = "Update Fail!...".mysqli_error($con);
